...
> 以下快速开始、Compose 和扩展开发命令为上游参考。本项目生产部署使用 [本项目生产部署](#本项目生产部署) 所述的 `mienvirtuoso/new-api` 镜像及外部 PostgreSQL、Redis；日常开发使用 `scripts/dev-local.sh start`（前端端口 3002），不使用下文的应用容器开发示例。
...
Dockerfile 使用 Bun 构建 `web/` 前端，再由 Go 编译后端并嵌入静态资源。
