...
func SetApiRouter(router *gin.Engine) {
	...
	apiRouter.Use(middleware.PaginationBounds())
	anonymousRequestBodyLimit := middleware.AnonymousRequestBodyLimit()
	{
		apiRouter.GET("/setup", controller.GetSetup)
		apiRouter.POST("/setup", anonymousRequestBodyLimit, controller.PostSetup)
		apiRouter.GET("/status", controller.GetStatus)
		apiRouter.GET("/uptime/status", middleware.HeaderNavModulePublicOrUserAuth("siteStatus"), controller.GetUptimeKumaStatus)
		...
		apiRouter.PUT("/status-monitor/cache/health-thresholds", middleware.AdminAuth(), controller.UpdateMonitorHealthThresholds)
		apiRouter.PUT("/status-monitor/cache/groups", middleware.AdminAuth(), controller.UpdateCacheMonitorGroups)
		apiRouter.GET("/status-monitor/providers", middleware.HeaderNavModulePublicOrUserAuth("siteStatus"), controller.GetOfficialProviderStatuses)
		...
		sso := apiRouter.Group("/sso/chatgpt2api", middleware.DisableCache())
		sso.GET("/start", middleware.UserAuth(), controller.StartChatGPT2APISSO)
		...
		userRoute := apiRouter.Group("/user")
		{
			...
			adminRoute.Use(middleware.AdminAuth())
			{
				...
				adminRoute.GET("/group_names", controller.GetUserGroupNames)
				adminRoute.GET("/topup", controller.GetAllTopUps)
				adminRoute.GET("/billing", controller.GetAllBillingHistory)
				adminRoute.GET("/aff/rewards/all", controller.GetAllAffiliateRewards)
		...
		optionRoute.Use(middleware.RootAuth())
		{
			...
			optionRoute.POST("/bulk", controller.UpdateOptionsBulk)
			optionRoute.POST("/group", controller.UpdateGroupSettings)
