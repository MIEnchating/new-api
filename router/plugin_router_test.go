...
func TestWebCacheHeadersDoNotLeakOntoPluginRoutes(t *testing.T) {
	...
	assert.NotEmpty(t, fallbackResponse.Header().Get("Cache-Control"))
...
func newOuterPluginTestEngine() *gin.Engine {
	...
	outer.Use(func(c *gin.Context) {
		...
		requestContext := context.WithValue(c.Request.Context(), constant.ContextKeyRequestId, "request-phase-two")
