...
> 以下快速开始、Compose 和扩展开发命令为上游参考。本二开版本的生产入口与宿主机开发步骤见 [本项目 README](./README.md#本项目生产部署)：生产镜像为 `mienvirtuoso/new-api`，数据库和 Redis 是外部服务；日常开发使用 `scripts/dev-local.sh start`（前端端口 3002），不使用下文的应用容器开发示例。
...
仓库的 [Compose 配置](https://github.com/QuantumNous/new-api/blob/main/docker-compose.yml) 默认启动 **New API + PostgreSQL + Redis**，并提供 MySQL 和独立 ClickHouse 日志库的配置示例。
