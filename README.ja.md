...
> 以下のクイックスタート、Compose、拡張開発のコマンドは上流プロジェクトの参考例です。このフォークでは[デプロイガイド](./README.md#本项目生产部署)に従い、`mienvirtuoso/new-api` と外部の PostgreSQL・Redis を使用してください。日常開発は `scripts/dev-local.sh start` でホスト上のプロセスを起動します（フロントエンドはポート 3002）。以下のアプリコンテナによる開発例は使用しません。
...
リポジトリの [Compose 設定](https://github.com/QuantumNous/new-api/blob/main/docker-compose.yml) は、既定で **New API + PostgreSQL + Redis** を起動します。MySQL と独立した ClickHouse ログデータベースの設定例も含まれます。
