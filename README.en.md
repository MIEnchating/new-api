...
> The quick-start, Compose, and extension development commands below are upstream reference examples. For this fork, follow the [fork deployment guide](./README.md#本项目生产部署): use `mienvirtuoso/new-api` with external PostgreSQL and Redis. For daily development, use host processes via `scripts/dev-local.sh start` (frontend port 3002), not the application-container development examples below.
...
The repository's [Compose configuration](https://github.com/QuantumNous/new-api/blob/main/docker-compose.yml) starts **New API + PostgreSQL + Redis** by default. It also contains examples for MySQL and a separate ClickHouse log database.
