...
func GetFlowQuotaData(startTime int64, endTime int64, username string, tokenName string, userID int, role int) ([]*FlowQuotaData, error) {
	...
	default:
		return getSelfFlowQuotaData(startTime, endTime, tokenName, userID)
	}
}

func flowQuotaBaseQuery(startTime int64, endTime int64, tokenName string) *gorm.DB {
	query := DB.Table("quota_data").
		Where("use_group <> ''").
		Where("created_at >= ? and created_at <= ?", startTime, endTime)
	return applyQuotaTokenNameFilter(query, tokenName)
}

func getSelfFlowQuotaData(startTime int64, endTime int64, tokenName string, userID int) ([]*FlowQuotaData, error) {
	rows := make([]*FlowQuotaData, 0)
	err := flowQuotaBaseQuery(startTime, endTime, tokenName).
...
func getAdminFlowQuotaData(startTime int64, endTime int64, username string, tokenName string) ([]*FlowQuotaData, error) {
	rows := make([]*FlowQuotaData, 0)
	query := flowQuotaBaseQuery(startTime, endTime, tokenName).
