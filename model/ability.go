...
func GetChannelWithConstraints(
	group string,
	model string,
	retry int,
...
func GetChannelWithFilter(group string, model string, retry int, requestPath string, filter ChannelCandidateFilter) (*Channel, error) {
	...
	snapshot, err := loadSatisfiedChannelSelectionSnapshotFromDB(group, model, filters)
	if err != nil {
		return nil, err
	}
...
func loadSatisfiedChannelSelectionSnapshotFromDB(group string, model string, filters []dto.ChannelFilter) (*ChannelSelectionSnapshot, error) {
	var abilities []Ability
	...
	abilities = filterAbilitiesByConstraintsWithChannels(abilities, model, filters, channelMap)
	if len(abilities) == 0 {
		return newChannelSelectionSnapshot(nil, false), nil
	}
...
func filterAbilitiesByRequestPathAndModel(abilities []Ability, requestPath string, model string) []Ability {
	return filterAbilitiesByConstraints(abilities, model, []dto.ChannelFilter{{
...
func filterAbilitiesByConstraints(abilities []Ability, modelName string, filters []dto.ChannelFilter) []Ability {
	...
	return filterAbilitiesByConstraintsWithChannels(abilities, modelName, filters, channelsByID)
}

func filterAbilitiesByConstraintsWithChannels(abilities []Ability, modelName string, filters []dto.ChannelFilter, channelsByID map[int]*Channel) []Ability {
