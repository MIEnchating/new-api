...
> Les commandes de démarrage rapide, Compose et développement ci-dessous sont des exemples du projet amont. Pour ce fork, suivez le [guide de déploiement](./README.md#本项目生产部署) : image `mienvirtuoso/new-api`, PostgreSQL et Redis externes. Pour le développement quotidien, utilisez les processus hôtes avec `scripts/dev-local.sh start` (frontend sur le port 3002), plutôt que les exemples de conteneur applicatif ci-dessous.
...
La [configuration Compose](https://github.com/QuantumNous/new-api/blob/main/docker-compose.yml) du dépôt démarre **New API + PostgreSQL + Redis** par défaut. Elle contient aussi des exemples pour MySQL et une base de journaux ClickHouse distincte.
