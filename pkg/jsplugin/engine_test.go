...
`, Options{})
	...
	disabledContext := context.WithValue(context.Background(), constant.ContextKeyRequestId, "plugin-console-disabled")
	_, err = engine.Call(disabledContext, "run", "disabled")
	require.NoError(t, err)
	assert.Empty(t, output.String())

	common.DebugEnabled = true
	contextA := context.WithValue(context.Background(), constant.ContextKeyRequestId, "plugin-console-request-a")
	_, err = engine.Call(contextA, "run", "context-a")
	require.NoError(t, err)
	logA := output.String()
	output.Reset()

	contextB := context.WithValue(context.Background(), constant.ContextKeyRequestId, "plugin-console-request-b")
