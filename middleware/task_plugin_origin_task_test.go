...
func TestApplyOriginTaskIntent(t *testing.T) {
	...
	for _, testCase := range tests {
		t.Run(testCase.name, func(t *testing.T) {
			...
			pin, ok, conflicts := service.GetChannelConstraints(c).ResolvedPin()
			require.Empty(t, conflicts)
			require.True(t, ok)
			assert.Equal(t, testCase.wantPinned, pin.ChannelId)
			...
			for i, wantID := range testCase.wantIDs {
				...
				assert.Equal(t, "upstream-"+wantID, tasks[i].PrivateData.UpstreamTaskID)
				assert.Equal(t, "text_to_video", tasks[i].Action)
				assert.Equal(t, model.TaskStatus(model.TaskStatusSuccess), tasks[i].Status)
				assert.JSONEq(t, fmt.Sprintf(`{"id":"upstream-%s"}`, wantID), string(tasks[i].Data))
			}
		})
	}
}
