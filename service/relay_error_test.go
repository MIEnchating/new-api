...
func TestRequestPolicyEventsReachLogAdminInfo(t *testing.T) {
	...
	assert.Empty(t, state.Events(), "channel selection belongs to the execution trace")
	...
	assert.Equal(t, PolicyDecision{Action: "stop", Reason: "attempt_budget_exhausted", Source: "global"}, events[0].Decision)
	assert.Equal(t, http.StatusUnauthorized, events[0].Status)
	assert.Equal(t, string(types.ErrorCodeBadResponseStatusCode), events[0].ErrorCode)
	assert.Equal(t, "upstream", events[0].ErrorSource)
	assert.Equal(t, 1, events[0].Attempt)
	assert.Equal(t, 7, events[0].ChannelID)
	assert.Equal(t, "default", events[0].Group)
	assert.Equal(t, "channel_disable_requested", events[0].Health, "the health entry follows the automatic disable rules")
	common.SetContextKey(c, constant.ContextKeyChannelIsMultiKey, true)
	RecordPolicyFailure(c, 7, apiErr, DecideRelayRetry(c, apiErr, 0))
	events = state.Events()
	require.Len(t, events, 2)
	assert.Equal(t, "key_disable_requested", events[1].Health)
	...
	require.Len(t, events, 3, "the outcome is recorded once")
	assert.Equal(t, PolicyDecision{Action: "success", Reason: "request_completed", Source: "upstream"}, events[2].Decision)
	assert.Equal(t, 8, events[2].ChannelID)
