...
func TestIntValueRejectsOverflowWithoutChangingValue(t *testing.T) {
	for _, input := range []string{
		`1e100`, `-1e100`, `9223372036854775808`, `-9223372036854775809`,
	} {
		t.Run(input, func(t *testing.T) {
			value := IntValue(7)
			require.Error(t, kitutil.UnmarshalJsonStr(input, &value))
			assert.Equal(t, IntValue(7), value)
		})
	}
}
