...
func TestGetTaskAdaptorForRequestUsesSoraProviderCompatibility(t *testing.T) {
	c, _ := gin.CreateTestContext(httptest.NewRecorder())
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/videos", nil)
	common.SetContextKey(c, constant.ContextKeyChannelBaseUrl, "https://api.kie.ai")
	common.SetContextKey(c, constant.ContextKeyOriginalModel, "wan/2-7-videoedit")

	platform, adaptor := getTaskAdaptorForRequest(c, constant.TaskPlatform(strconv.Itoa(constant.ChannelTypeSora)))

	require.NotNil(t, adaptor)
	assert.Equal(t, constant.TaskPlatformSoraCompat, platform)
	assert.Equal(t, "sora-compat", adaptor.GetChannelName())
	_, pinned := c.Get(pluginruntime.ContextKeyPinnedPlugin)
	assert.False(t, pinned)
}

func TestGetTaskAdaptorRejectsSoraProviderCompatibilityWhenTaskPluginsDisabled(t *testing.T) {
	originalEnabled := pluginruntime.DefaultRegistry.Enabled()
	pluginruntime.DefaultRegistry.SetEnabled(false)
	t.Cleanup(func() { pluginruntime.DefaultRegistry.SetEnabled(originalEnabled) })

	assert.Nil(t, GetTaskAdaptor(constant.TaskPlatformSoraCompat))
...
func TestGetTaskAdaptorForRequestKeepsPinnedSoraPluginForProtocolEndpoint(t *testing.T) {
	plugin, found := pluginruntime.DefaultRegistry.Generation().Get("sora")
...
func TestGetTaskAdaptorForRequestOverridesPinnedVideoForAPIMart(t *testing.T) {
	plugin, found := pluginruntime.DefaultRegistry.Generation().Get("sora")
	require.True(t, found)
	c, _ := gin.CreateTestContext(httptest.NewRecorder())
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/videos", nil)
	c.Set(pluginruntime.ContextKeyPinnedPlugin, pluginruntime.PinnedPlugin{Plugin: plugin})
	c.Set(pluginruntime.ContextKeyPinnedEndpoint, pluginruntime.PinnedEndpoint{Plugin: plugin, Protocol: "openai_video"})
	common.SetContextKey(c, constant.ContextKeyChannelBaseUrl, "https://api.apimart.ai")
	common.SetContextKey(c, constant.ContextKeyOriginalModel, "sora-2")

	platform, adaptor := getTaskAdaptorForRequest(c, constant.TaskPlatform("sora"))
