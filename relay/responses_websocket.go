...
func (s *responsesWSSession) runCall(c *gin.Context, state *responsesWSCallState, create responsesWSCreateRequest) (apiErr *types.NewAPIError) {
	...
	if s.lockedChannelID != 0 {
		...
		service.AppendUsedChannel(c, s.lockedChannelID)
		...
		for {
			...
			service.TrackResolvedChannelExecutionAttempt(c, common.GetContextKeyString(c, appconstant.ContextKeyUsingGroup), modelName, c.Request.URL.Path, channel, retry.GetRetry())
			if info == nil {
				info = relaycommon.GenRelayInfoResponses(c, &create.Request)
				info.IsStream = true
				common.SetContextKey(c, appconstant.ContextKeyIsStream, true)
			...
			if dialErr != nil {
				...
				sameChannelRetries = 0
				willRetry := decision.Action == "retry" && retry.GetRetry() < common.RetryTimes
