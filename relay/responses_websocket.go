...
type responsesWSSession struct {
	...
	lockedUpstreamRequestID string
}
...
func (s *responsesWSSession) runCall(c *gin.Context, state *responsesWSCallState, create responsesWSCreateRequest) (apiErr *types.NewAPIError) {
	...
	defer func() {
		...
		if info != nil && apiErr != nil {
			...
			if !failureHandled && c.Request.Context().Err() == nil {
				service.TrackChannelExecutionFailure(c, c.GetInt("channel_id"), apiErr.ErrorWithStatusCode())
				service.HandleChannelFailure(c, *types.NewChannelError(c.GetInt("channel_id"), c.GetInt("channel_type"), c.GetString("channel_name"), common.GetContextKeyBool(c, appconstant.ContextKeyChannelIsMultiKey), common.GetContextKeyString(c, appconstant.ContextKeyChannelKey), c.GetBool("auto_ban")), apiErr)
			}
			service.RecordRelayErrorLog(c, c.GetInt("channel_id"), apiErr, info, false)
		}
	}()
	if modelName == "" {
	...
	if s.lockedChannelID != 0 {
		...
		addResponsesWSUsedChannel(c, s.lockedChannelID)
		service.TrackResolvedChannelExecutionAttempt(c, s.lockedGroup, modelName, c.Request.URL.Path, &appmodel.Channel{Id: s.lockedChannelID, Name: c.GetString("channel_name")}, 0)
		c.Set(common.UpstreamRequestIdKey, s.lockedUpstreamRequestID)
		info.IsStream = true
		...
		var sameChannel *appmodel.Channel
		...
		for {
			...
			if sameChannel != nil {
				channel = sameChannel
				sameChannel = nil
				apiErr = middleware.SetupContextForSelectedChannel(c, channel, modelName)
			} else {
				channel, apiErr = selectResponsesWSChannel(c, modelName, retry)
			}
			if apiErr != nil {
				if info != nil && info.LastError != nil && (service.IsChannelRouteEnabled() || service.HasTokenGroupRoutes(c)) {
			...
			service.TrackResolvedChannelExecutionAttempt(c, info.UsingGroup, modelName, c.Request.URL.Path, channel, retry.GetRetry())
			var payload []byte
			payload, apiErr = buildResponsesWSCreatePayload(c, info, create.Request, create.Generate)
			if apiErr != nil {
				return apiErr
			...
			if apiErr != nil {
				...
				service.TrackChannelExecutionFailure(c, channel.Id, apiErr.ErrorWithStatusCode())
				if service.ShouldRetrySameChannelRouteForContext(c, apiErr, sameChannelRetries) {
					sameChannelRetries++
					sameChannel = channel
					service.TrackChannelExecutionSameChannelRetry(c, channel, sameChannelRetries)
					service.RecordRelayErrorLog(c, channel.Id, apiErr, info, true)
					continue
				}
				failureHandled = true
				routeAdvanced := service.HandleChannelFailure(c, *types.NewChannelError(channel.Id, channel.Type, channel.Name, channel.ChannelInfo.IsMultiKey, info.ApiKey, channel.GetAutoBan()), apiErr)
				sameChannelRetries = 0
				willRetry := routeAdvanced
				if !service.IsChannelRouteEnabled() && !service.HasTokenGroupRoutes(c) {
					willRetry = service.ShouldRetryRelayError(c, apiErr, common.RetryTimes-retry.GetRetry())
				}
				if willRetry {
					service.RecordRelayErrorLog(c, channel.Id, apiErr, info, true)
					retry.IncreaseRetry()
			...
			s.lockedUpstreamRequestID = c.GetString(common.UpstreamRequestIdKey)
	...
	for {
		...
		case incoming := <-state.inbox:
			...
			if incoming.err != nil {
				...
				state.closeAfter = true
				if !accepted {
					return types.NewErrorWithStatusCode(incoming.err, types.ErrorCodeBadResponse, http.StatusBadGateway, types.ErrOptionWithSkipRetry())
				}
				settleResponsesWSUsage(c, info, accumulator.Finish())
				return nil
			}
			info.SetFirstResponseTime()
			info.ObserveActualResponseModel(incoming.body)
		...
		case <-idle.C:
			...
			state.closeAfter = true
			if !accepted {
				return types.NewErrorWithStatusCode(context.DeadlineExceeded, types.ErrorCodeBadResponse, http.StatusGatewayTimeout, types.ErrOptionWithSkipRetry())
			}
			settleResponsesWSUsage(c, info, accumulator.Finish())
			return nil
		case <-s.ctx.Done():
			...
			if !accepted {
				return types.NewError(s.ctx.Err(), types.ErrorCodeBadResponse, types.ErrOptionWithSkipRetry())
			}
			settleResponsesWSUsage(c, info, accumulator.Finish())
			return nil
		}
...
func settleResponsesWSUsage(c *gin.Context, info *relaycommon.RelayInfo, usage *dto.Usage) {
	status := info.StreamStatus
	if status != nil && status.EndReason != relaycommon.StreamEndReasonClientGone && (!status.IsNormalEnd() || status.HasErrors()) {
		streamErr := status.FailureError()
		if streamErr == nil {
			streamErr = errors.New("upstream response stream failed")
		}
		apiErr := types.NewOpenAIError(streamErr, types.ErrorCodeBadResponse, http.StatusBadGateway, types.ErrOptionWithStreamEvent())
		service.TrackChannelExecutionFailure(c, info.ChannelId, apiErr.ErrorWithStatusCode())
		service.HandleChannelFailure(c, *types.NewChannelError(info.ChannelId, info.ChannelType, c.GetString("channel_name"), info.ChannelIsMultiKey, info.ApiKey, c.GetBool("auto_ban")), apiErr)
	}
...
func (s *responsesWSSession) restoreConnectionContext(c *gin.Context, model string) *types.NewAPIError {
	channel, err := service.ValidateWebSocketChannel(s.lockedChannelID, s.lockedKey, s.lockedKeyIndex)
	if err != nil {
		return types.NewErrorWithStatusCode(err, types.ErrorCodeAccessDenied, http.StatusForbidden, types.ErrOptionWithSkipRetry())
	}
	if !channel.GetSetting().ResponsesWebSocketEnabled {
	...
	if service.HasTokenGroupRoutes(c) {
		if !service.RestoreTokenGroupRoute(c, s.lockedGroup, model, c.Request.URL.Path) {
		...
		group = s.lockedGroup
	} else if group == "auto" && !(pinned && s.lockedGroup == group) {
	...
	} else if group != s.lockedGroup {
		return types.NewErrorWithStatusCode(errors.New("the connection group changed; reconnect required"), types.ErrorCodeAccessDenied, http.StatusForbidden, types.ErrOptionWithSkipRetry())
	}
	if pinned {
	...
	service.TrackChannelRouteSelection(c, group, model, c.Request.URL.Path, channel.Id)
	return nil
}
...
func dialResponsesWebSocketUpstream(c *gin.Context, adaptor relaychannel.Adaptor, info *relaycommon.RelayInfo) (*websocket.Conn, *types.NewAPIError) {
	...
	if resp != nil {
		...
		if info.ChannelType == appconstant.ChannelTypeSub2API {
			c.Set(common.UpstreamRequestIdKey, c.GetString(common.RequestIdKey))
		}
	}
	if err != nil {
...
func selectResponsesWSChannel(c *gin.Context, modelName string, retryParam *service.RetryParam) (*appmodel.Channel, *types.NewAPIError) {
	...
	if retryParam.GetRetry() == 0 && !service.IsChannelRouteEnabled() && !service.HasTokenGroupRoutes(c) && usingGroup != "auto" {
