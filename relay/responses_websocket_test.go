...
func TestResponsesWSRestoreConnectionPreservesAuthorizedBillingGroup(t *testing.T) {
	database := setupRelayChannelDB(t)
	require.NoError(t, database.AutoMigrate(&model.Ability{}))
	channel := &model.Channel{Name: "route-fallback", Key: "fixture-key", Type: constant.ChannelTypeOpenAI, Status: common.ChannelStatusEnabled, Group: "fallback", Models: "ws-route"}
	channel.SetSetting(dto.ChannelSettings{ResponsesWebSocketEnabled: true})
	require.NoError(t, database.Create(channel).Error)
	require.NoError(t, database.Create(&model.Ability{ChannelId: channel.Id, Model: "ws-route", Group: "fallback", Enabled: true}).Error)
	common.MemoryCacheEnabled = true
	model.InitChannelCache()
	t.Cleanup(func() {
		require.NoError(t, database.Where("channel_id = ?", channel.Id).Delete(&model.Ability{}).Error)
		require.NoError(t, database.Delete(channel).Error)
		model.InitChannelCache()
	})
	previousRatios := ratio_setting.GroupRatio2JSONString()
	require.NoError(t, ratio_setting.UpdateGroupRatioByJSONString(`{"primary":1,"fallback":2}`))
	t.Cleanup(func() { require.NoError(t, ratio_setting.UpdateGroupRatioByJSONString(previousRatios)) })
	session := &responsesWSSession{
		lockedModel: "ws-route", lockedChannelID: channel.Id, lockedGroup: "fallback", lockedKey: channel.Key,
		lockedContext: map[constant.ContextKey]any{
			constant.ContextKeyChannelId: channel.Id, constant.ContextKeyChannelType: channel.Type,
			constant.ContextKeyChannelKey: channel.Key, constant.ContextKeyChannelBaseUrl: channel.GetBaseURL(),
	...
	for _, tc := range []struct {
		...
		{name: "fallback route remains allowed", routes: []model.TokenGroupRoute{{Group: "primary"}, {Group: "fallback"}}, valid: true},
		{name: "pinned fallback keeps its pricing group", pin: true, routes: []model.TokenGroupRoute{{Group: "primary"}, {Group: "fallback"}}, valid: true},
		{name: "removed fallback rejects", routes: []model.TokenGroupRoute{{Group: "primary"}}},
		{name: "pin cannot retain a revoked group", pin: true, routes: []model.TokenGroupRoute{{Group: "primary"}}},
		{name: "administrator pin preserves auto group", pin: true, auto: true, valid: true},
		{name: "ordinary auto cannot retain unresolved group", auto: true},
	} {
		t.Run(tc.name, func(t *testing.T) {
			c, _ := gin.CreateTestContext(httptest.NewRecorder())
			c.Request = httptest.NewRequest(http.MethodPost, "/v1/responses", nil)
			common.SetContextKey(c, constant.ContextKeyUsingGroup, "primary")
			common.SetContextKey(c, constant.ContextKeyTokenGroupRoutes, tc.routes)
			session.lockedGroup = "fallback"
			if tc.auto {
				common.SetContextKey(c, constant.ContextKeyUsingGroup, "auto")
				session.lockedGroup = "auto"
			}
			if tc.pin {
			...
			apiErr := session.restoreConnectionContext(c, "ws-route")
			if !tc.valid {
			...
			info := relaycommon.GenRelayInfoResponses(c, &dto.OpenAIResponsesRequest{Model: "ws-route"})
			...
			assert.Equal(t, float64(2), ratio.GroupRatio)
		})
	}
	// Persisted status wins even if a node still has an older channel cache.
	require.NoError(t, database.Model(channel).Update("status", common.ChannelStatusManuallyDisabled).Error)
	_, err := service.ValidateWebSocketChannel(channel.Id, channel.Key, 0)
	require.Error(t, err)
}
