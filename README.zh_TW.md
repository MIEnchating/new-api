...
> 以下快速開始、Compose 與擴充開發指令為上游參考。本分支的正式部署請依照[部署指南](./README.md#本项目生产部署)，使用 `mienvirtuoso/new-api` 映像及外部 PostgreSQL、Redis。日常開發使用 `scripts/dev-local.sh start` 啟動主機程序（前端連接埠 3002），不使用下方的應用程式容器開發範例。
...
儲存庫的 [Compose 設定](https://github.com/QuantumNous/new-api/blob/main/docker-compose.yml) 預設啟動 **New API + PostgreSQL + Redis**，並提供 MySQL 和獨立 ClickHouse 日誌資料庫的設定範例。
