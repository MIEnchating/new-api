...
func setupMultiKeyManagementTest(t *testing.T) (*gorm.DB, *model.User) {
	t.Helper()
...
func TestMultiKeyEnableRestoresOnlyExhaustedChannels(t *testing.T) {
	database, root := setupMultiKeyManagementTest(t)
...
func TestMultiKeyDeletingAllDisabledKeysDoesNotRestoreCredentials(t *testing.T) {
	database, root := setupMultiKeyManagementTest(t)
	for _, cacheEnabled := range []bool{false, true} {
		for _, initialStatus := range []int{common.ChannelStatusEnabled, common.ChannelStatusManuallyDisabled} {
			t.Run(fmt.Sprintf("cache=%t/status=%d", cacheEnabled, initialStatus), func(t *testing.T) {
				previousCache := common.MemoryCacheEnabled
				common.MemoryCacheEnabled = cacheEnabled
				t.Cleanup(func() { common.MemoryCacheEnabled = previousCache })
				channel := &model.Channel{
					Name: t.Name(), Type: 1, Key: "key-one\nkey-two", Status: initialStatus, Models: "test-model", Group: "default",
					ChannelInfo: model.ChannelInfo{IsMultiKey: true, MultiKeySize: 2, MultiKeyStatusList: map[int]int{
						0: common.ChannelStatusAutoDisabled, 1: common.ChannelStatusAutoDisabled,
					}},
				}
				require.NoError(t, channel.Insert())
				t.Cleanup(func() {
				...
				for _, action := range []string{"delete_disabled_keys", "enable_key", "enable_all_keys"} {
					...
					require.NoError(t, common.Unmarshal(recorder.Body.Bytes(), &result))
					assert.Equal(t, action != "enable_key", result.Success, recorder.Body.String())
					loaded, err := model.GetChannelById(channel.Id, true)
					require.NoError(t, err)
					assert.Empty(t, loaded.Key, action)
					assert.Empty(t, loaded.GetKeys(), action)
					assert.Zero(t, loaded.ChannelInfo.MultiKeySize, action)
					assert.Empty(t, loaded.ChannelInfo.MultiKeyStatusList, action)
					assert.Equal(t, common.ChannelStatusManuallyDisabled, loaded.Status, action)
					cached, err := model.CacheGetChannel(channel.Id)
					require.NoError(t, err)
					assert.Empty(t, cached.Key, action)
					assert.Empty(t, cached.GetKeys(), action)
					assert.Equal(t, common.ChannelStatusManuallyDisabled, cached.Status, action)
					key, _, apiErr := loaded.GetNextEnabledKey()
					assert.Empty(t, key, action)
					require.NotNil(t, apiErr, action)
					var ability model.Ability
					require.NoError(t, database.Where("channel_id = ?", channel.Id).First(&ability).Error)
					assert.False(t, ability.Enabled, action)
				}
			})
		}
	}
