...
func listModelsMeta(c *gin.Context, keyword, vendor string) {
	...
	if squareState != "" {
		// The shared fork parser normalizes invalid pagination. Validate the
		// explicit values first to retain the visibility filter's API contract.
		for _, key := range []string{"p", "page_size", "ps", "size"} {
			if value, err := strconv.Atoi(c.Query(key)); err == nil && value < 1 {
