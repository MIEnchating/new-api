...
func testChannelForHealthCheck(ctx context.Context, channel *model.Channel, testUserID int, allowDisable bool, disableThreshold int64) channelTestSummary {
	...
	if allowDisable && isChannelEnabled && shouldBanChannel && channel.GetAutoBan() {
		...
		recordChannelErrorLog(result.context, newAPIError, nil, true)
		summary.Disabled++
	}
...
// concurrency. Results and progress are reduced by the caller goroutine.
func runChannelTestWorkers(ctx context.Context, channels []*model.Channel, concurrency int, run func(context.Context, *model.Channel) channelTestSummary, report func(processed, total int)) channelTestSummary {
	if ctx == nil {
		ctx = context.Background()
	}
	total := len(channels)
...
// concurrency and honors cancellation when a system-task runner loses its lease.
func performChannelTests(ctx context.Context, channels []*model.Channel, testUserID int, allowDisable bool, concurrency int, report func(processed, total int)) channelTestSummary {
	...
	return runChannelTestWorkers(ctx, channels, concurrency, func(ctx context.Context, channel *model.Channel) channelTestSummary {
