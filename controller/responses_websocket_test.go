...
func newResponsesWSBillingTest(t *testing.T, expression string, handle func(*websocket.Conn, *http.Request)) *responsesWSBillingTest {
	...
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		ws, err := (&websocket.Upgrader{}).Upgrade(w, r, http.Header{"X-Request-Id": []string{"upstream-ws-handshake"}})
...
func TestResponsesWebSocketReusesConnectionAndSettlesEachRequest(t *testing.T) {
	...
	for index, expectedQuota := range []int{1000, 2000} {
		...
		assert.Equal(t, "upstream-ws-handshake", logs[index].UpstreamRequestId)
		require.NotNil(t, logs[index].ActualResponseModel)
		assert.Equal(t, "ws-billing", *logs[index].ActualResponseModel)
		...
		assert.Equal(t, map[string]any{"status": "ok", "end_reason": "done"}, other["stream_status"])
	}
...
func TestResponsesWebSocketInitialUpstreamRejectionRefundsReservation(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "token priced rejection", expression: `tier("output", c * 2)`, status: http.StatusBadRequest},
		{name: "request priced rejection", expression: `tier("request", fixed(0.00002))`, status: http.StatusBadRequest},
		{name: "disconnect before acceptance", expression: `tier("request", fixed(0.00002))`, disconnect: true, status: http.StatusBadGateway},
	} {
		t.Run(tc.name, func(t *testing.T) {
