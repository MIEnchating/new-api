...
func ManageMultiKeys(c *gin.Context) {
	...
	case "delete_disabled_keys":
		...
		shouldCloseWebSocket := disableMultiKeyChannelIfUnavailable(channel)
		if len(remainingKeys) == 0 {
			// An explicit deletion must persist an empty key instead of using
			// the ordinary channel edit convention that retains an omitted key.
			err = model.DB.Transaction(func(tx *gorm.DB) error {
				if err := tx.Model(channel).Select("key", "channel_info", "status", "other_info").Updates(channel).Error; err != nil {
					return err
				}
