...
func TestChatGPT2APISSO(t *testing.T) {
	...
	for _, issuer := range cfg.Issuers {
		t.Run(issuer, func(t *testing.T) {
			...
			require.NoError(t, err)
			// The SSO payload must preserve administrator status for relying apps.
			require.NoError(t, model.DB.Model(&model.User{}).Where("id = ?", user.Id).Update("role", common.RoleAdminUser).Error)
