	...
	for _, issuer := range cfg.Issuers {
		t.Run(issuer, func(t *testing.T) {
			...
			for _, role := range []int{common.RoleAdminUser, common.RoleRootUser} {
				require.NoError(t, model.DB.Model(&model.User{}).Where("id = ?", user.Id).Update("role", role).Error)
				adminTarget, err := cfg.Authorize(request, issuer, identity)
				require.NoError(t, err)
				adminCallback, err := url.Parse(adminTarget)
				require.NoError(t, err)
				adminAccount, err := cfg.Exchange(adminCallback.Query().Get("code"), verifier, issuer)
				require.NoError(t, err)
				assert.True(t, adminAccount.IsAdmin, "role %d must grant administration on first exchange", role)
				adminAccount, err = cfg.Validate(account.Reference, issuer)
				require.NoError(t, err)
				assert.True(t, adminAccount.IsAdmin, "role %d must refresh existing SSO permissions", role)
			}
			require.NoError(t, model.DB.Model(&model.User{}).Where("id = ?", user.Id).Update("role", common.RoleCommonUser).Error)
			demotedAccount, err := cfg.Validate(account.Reference, issuer)
			require.NoError(t, err)
			assert.False(t, demotedAccount.IsAdmin, "removed administration must not survive in an existing SSO session")
