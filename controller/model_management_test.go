		...
		t.Run(dialect.kind, func(t *testing.T) {
			...
			t.Run("square_states_follow_catalog_policy", func(t *testing.T) {
				...
				t.Run("filter_square_state_before_pagination", func(t *testing.T) {
					...
					for _, query := range []string{"p=-1", "page_size=-1", "p=0", "page_size=0", "ps=-1", "size=-1"} {
			...
			t.Run("pricing_saves_zero_switches_modes_and_rejects_stale_batches", func(t *testing.T) {
				...
				perSecond := model.ModelPricingChange{
					ModelName:       "matrix-priced",
					ExpectedVersion: loaded.Entries[0].Version,
					Pricing: model.PricingValues{
						"ModelPrice":                   float64(0.4),
						"billing_setting.billing_mode": billing_setting.BillingModePerSecond,
					},
				}
				require.NoError(t, model.UpdateModelPricing([]model.ModelPricingChange{perSecond}))
				loaded, err = model.GetModelPricingSnapshot([]string{"matrix-priced"})
				require.NoError(t, err)
				assert.Equal(t, float64(0.4), loaded.Entries[0].Effective["ModelPrice"])
				assert.Equal(t, billing_setting.BillingModePerSecond, loaded.Entries[0].Effective["billing_setting.billing_mode"])
				stale := changes[0]
				...
				invalid.Pricing = model.PricingValues{"billing_setting.billing_mode": billing_setting.BillingModePerSecond}
				assert.Error(t, model.UpdateModelPricing([]model.ModelPricingChange{invalid}))
