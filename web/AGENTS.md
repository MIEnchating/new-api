...
- 启动开发或验证服务前，必须先检查本项目已有前端进程、监听端口、systemd 服务和 Docker 容器，并确认工作目录与服务用途。
- 已有配置匹配且可用的前端开发服务必须直接复用；同一项目、同一服务类型和同一端口只允许一个实例，禁止重复执行 `bun run dev`。
...
- 重启或停止服务前必须核对 PID、工作目录、监听端口及所属 systemd/cgroup，普通前端修改优先依赖 HMR，不得无故重启已有服务。
