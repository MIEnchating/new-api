...
export function buildLinuxDOOAuthUrl(
  ...
  url.searchParams.set('client_id', clientId)
  url.searchParams.set('state', state)
  url.searchParams.set('redirect_uri', `${origin}/oauth/linuxdo`)
  return url.toString()
}
...
export async function handleDiscordOAuth(
  ...
  const url = buildDiscordOAuthUrl(clientId, state, callbackOrigin)
  window.open(url, '_blank')
}
...
export async function handleOIDCOAuth(
  ...
  const url = buildOIDCOAuthUrl(authUrl, clientId, state, callbackOrigin)
  window.open(url, '_blank')
}
...
export async function handleLinuxDOOAuth(
  ...
  const url = buildLinuxDOOAuthUrl(clientId, state, callbackOrigin)
  window.open(url, '_blank')
}
