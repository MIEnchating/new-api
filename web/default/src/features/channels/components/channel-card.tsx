...
function ChannelCardComponent({
  ...
  const showType = visibleColumnIds.has('type')
  const showRouteStatus =
    !isTagRow && visibleColumnIds.has('route_status') && routeStatusCell
  const hasStatRows =
    showId ||
    showTag ||
    showTestTime ||
        ...
        >
          <div className='flex min-w-0 items-start gap-2.5'>
            ...
            <div className='min-w-0 flex-1'>
              ...
              {(showType || showRouteStatus) && (
                <div className='mt-1.5 flex min-w-0 flex-wrap items-center gap-1.5'>
                  {showType ? typeCell : null}
                  {showRouteStatus || null}
                </div>
              )}
            </div>
