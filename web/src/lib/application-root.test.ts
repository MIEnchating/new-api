...
import { replaceBootstrapAndMount } from './application-root'

test('the static bootstrap is removed before React mounts', () => {
  const events: string[] = []
  const rootElement = {
    replaceChildren: () => events.push('bootstrap-removed'),
  } as unknown as HTMLElement

  replaceBootstrapAndMount(rootElement, (element) => {
