...
it('preserves non-API audit fields in the mobile card', () => {
  renderLogs({
    logs: [
      {
        ...log,
        ...
        ip: '192.0.2.10',
        other: JSON.stringify({
          op: { action: 'user.security_verify' },
          login_method: 'password',
          second_factor_method: 'totp',
          admin_info: { auth_method: 'session', payment_method: 'stripe' },
          request_method: 'POST',
          request_route: '/api/user/verify',
          audit_info: { success: false, status: 403 },
        }),
      },
    ],
  })
  for (const [label, value] of [
    ['Operation ID', 'user.security_verify'],
    ['Login Method', 'Password'],
    ['Second-factor method', 'Authenticator code'],
    ['Authentication Method', 'Session'],
    ['Order Payment Method', 'Stripe'],
    ['Request', 'POST /api/user/verify'],
