...
export function CommonLogMobileCard<TData>(props: {
  ...
  const auditStatus = other?.audit_info?.status
  const auditSuccess =
    other?.audit_info?.success ??
    (auditStatus == null ? undefined : auditStatus < 400)
  let auditResult = ''
  if (log.type === LOG_TYPE_ENUM.LOGIN) {
    auditResult = t('Success')
  } else if (auditSuccess != null) {
    auditResult = auditSuccess ? t('Success') : t('Failed')
    if (auditStatus != null) auditResult += ` (${auditStatus})`
  }
  const auditDetails = displayable
    ? []
    : [
        [t('Operation ID'), other?.op?.action],
        [t('Login Method'), getLoginMethodLabel(other?.login_method, t)],
        [
          t('Second-factor method'),
          getSecondFactorMethodLabel(other?.second_factor_method, t),
        ],
        [
          t('Authentication Method'),
          getAuditAuthMethodLabel(other?.admin_info?.auth_method, t),
        ],
        [
        ...
        [
          t('Request'),
          [other?.request_method, other?.request_route || other?.request_path]
            .filter(Boolean)
            .join(' '),
        ],
        [t('IP Address'), log.ip],
        [t('Result'), auditResult],
      ]
