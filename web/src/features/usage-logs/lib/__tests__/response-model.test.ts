...
describe('isResponseModelMismatch', () => {
  ...
  test.each([
    { returned: 'vendor/requested', stored: true, expected: false },
    { returned: 'other', stored: false, expected: true },
  ])(
    'recomputes legacy rows with a stored mismatch=$stored from $returned',
    ({ returned, stored, expected }) => {
      const legacy = { ...observation(returned), mismatch: stored }
      expect(isResponseModelMismatch(legacy)).toBe(expected)
      expect(legacy.mismatch).toBe(stored)
    }
  )
})
