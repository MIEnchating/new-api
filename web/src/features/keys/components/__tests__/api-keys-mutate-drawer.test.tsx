...
describe('API keys mutate drawer routing mode', () => {
  ...
  test('switches between single-group and group-routing cards', async () => {
    ...
    const [singleGroupCard, groupRoutingCard] = modeCards
    if (!singleGroupCard || !groupRoutingCard) {
      throw new Error('Expected single-group and group-routing cards')
    }
    expect(singleGroupCard).toHaveAttribute('aria-checked', 'true')
    expect(groupRoutingCard).toHaveAttribute('aria-checked', 'false')

    fireEvent.click(groupRoutingCard)
    expect(singleGroupCard).toHaveAttribute('aria-checked', 'false')
    expect(groupRoutingCard).toHaveAttribute('aria-checked', 'true')
    ...
    fireEvent.click(singleGroupCard)
    expect(singleGroupCard).toHaveAttribute('aria-checked', 'true')
    expect(groupRoutingCard).toHaveAttribute('aria-checked', 'false')
    expect(getControlByLabel('Group')).toBeInTheDocument()
  })
})
