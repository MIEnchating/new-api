    ...
    <I18nextProvider i18n={i18n}>
      <TooltipProvider>
        <ApiKeyGroupCell
          ...
          scheduleActive={props.scheduleActive}
          group={props.group}
          ratio={props.ratio}
...
describe('API key group table cell', () => {
  test('keeps the group and compact localized multiplier together with one subtle flowing edge', () => {
    ...
    expect(
      container.querySelector('[data-auto-group-frame]')
    ).toBeInTheDocument()
    const flow = container.querySelector('.auto-group-flow-border-subtle')
...
test('preserves scheduled ratio state alongside the compact group multiplier', () => {
  const { container } = render(
    <CellHarness group='premium' ratio={0.5} scheduleEnabled scheduleActive />
  )
  expect(screen.getByText('0.5x')).toBeInTheDocument()
  expect(
    container.querySelector('[data-group-ratio-schedule="active"]')
  ).toBeInTheDocument()
})
