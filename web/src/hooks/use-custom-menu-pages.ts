/*
Copyright (C) 2023-2026 QuantumNous

This program is free software: you can redistribute it and/or modify
it under the terms of the GNU Affero General Public License as
published by the Free Software Foundation, either version 3 of the
License, or (at your option) any later version.

This program is distributed in the hope that it will be useful,
but WITHOUT ANY WARRANTY; without even the implied warranty of
MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
GNU Affero General Public License for more details.

You should have received a copy of the GNU Affero General Public License
along with this program. If not, see <https://www.gnu.org/licenses/>.

For commercial licensing, please contact support@quantumnous.com
*/
import { queryOptions, useQuery } from '@tanstack/react-query'

import { api } from '@/lib/api'
import {
  parseCustomMenuPages,
  type CustomMenuPage,
} from '@/lib/custom-menu-pages'
import { useAuthStore } from '@/stores/auth-store'

type CustomMenuPagesResponse = {
  success: boolean
  data?: CustomMenuPage[]
}

export const customMenuPagesQueryOptions = (role: number) =>
  queryOptions({
    queryKey: ['custom-menu-pages', role],
    queryFn: async () => {
      const response = await api.get<CustomMenuPagesResponse>(
        '/api/custom-menu-pages'
      )
      return parseCustomMenuPages(response.data.data)
    },
    staleTime: 5 * 60 * 1000,
  })

export function useCustomMenuPages() {
  const role = useAuthStore((state) => state.auth.user?.role ?? 0)
  return useQuery(customMenuPagesQueryOptions(role))
}
